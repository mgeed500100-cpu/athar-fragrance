...
ثيم سلة مستقل للعطور والعود والهدايا. إصدار تصميم وتكامل 0.3.0، يحتاج تحققًا حيًا قبل البيع.
واجهة تلقائية بالصور المدمجة: هيدر متكيف، هيرو سينمائي، بطاقات منتجات حقيقية، تصنيفات غير متساوية، رحلة عطر بخمس عبوات، هدايا ومحتوى تحريري وتقييمات سلة. التفاصيل والحدود في docs/STATUS.md.

## الفروع

- `master`: النسخة المجهزة للاستيراد بعد الفحص.
- `design/athar-v1`: التطوير والتصميم.
- `baseline/raed-initial`: نسخة البداية دون تعديلات.
...
المعاينة تحتاج Salla CLI وحساب شركاء سلة. نجاح البناء لا يثبت نجاح الاستيراد أو المراجعة المرئية داخل سلة.
الواجهة الجديدة تعمل افتراضيًا دون إضافة بنرات يدويًا. تفعيل إعداد استخدام ترتيب المحرر يعيد ترتيب عناصر المحرر السابق، ويمكن إضافة مكونات أثر منفردة.
رحلة العطر المدمجة تعرض خمس حالات: كريمي، وردي، عنبري، خمري، ليلي؛ والمكون المتقدم يسمح بصور ونوتات مخصصة.
تقليل الحركة والشاشة القصيرة يعرضان المحتوى ثابتًا. لا توجد أسعار أو تقييمات وهمية.
البطاقات تسحب منتجات المتجر المطابقة لكلمة «عطر» القابلة للتعديل. الصور الفنية ليست منتجات للبيع؛ الثيم لا ينشئ مخزونًا ولا يحول الملابس إلى عطور.

## مراجع المشروع

- [الاتجاه والمراحل](docs/PROJECT.md)
- [المراجع والمصادر](docs/REFERENCES.md)
- [اختبارات القبول](docs/QA.md)
- [حالة التحقق](docs/STATUS.md)
- [دليل التاجر بالعربية](docs/MERCHANT-GUIDE-AR.md)

المستودع مستقل عن Nasaq One. لا ترفع مفاتيح أو بيانات عملاء.
Issues قناة تطوير مؤقتة؛ بريد noreply ليس بريد دعم. نجهز قناة دعم فعلية قبل النشر.

## Upstream documentation (retained for attribution)

The following is the original starter documentation, not a description of completed Athar features.
